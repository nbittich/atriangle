...
public interface ElasticSearchRdfService {
  ...
  default Settings.Builder inputStreamToSettingsJson(InputStream is) {
    return Settings.builder().loadFromSource(inputStreamToString(is), XContentType.JSON);
  }
  ...
  default CreateIndexResponse createIndex(String index, InputStream source) {
    return createIndex(
        index,
        createIndexRequest ->
            createIndexRequest.source(inputStreamToString(source), XContentType.JSON));
  }
  ...
  @SneakyThrows
  default AcknowledgedResponse updateSettings(
      String index, String settings, boolean preserveIndex) {
  ...
  default CreateIndexResponse createIndex(
      String index, Function<CreateIndexRequest, CreateIndexRequest> requestTransformer) {
    try {
  ...
  @SneakyThrows
  default CreateIndexResponse createIndex(
      String index, InputStream settings, InputStream mapping, InputStream aliases) {
    return createIndex(
        index,
        createIndexRequest ->
            createIndexRequest
                .settings(inputStreamToSettingsJson(settings).build())
  ...
  default AcknowledgedResponse deleteIndex(String index) {
    try {
      DeleteIndexRequest request = new DeleteIndexRequest(index);
      return getClient().indices().delete(request, RequestOptions.DEFAULT);
    } catch (Exception e) {
  ...
  default void indexAsync(String index, Function<IndexRequest, IndexRequest> requestTransformer) {
    ...
    getClient()
        .indexAsync(
            requestTransformer.apply(request), RequestOptions.DEFAULT, getDefaultIndexCallback());
  }

  default IndexResponse index(
      String index, Function<IndexRequest, IndexRequest> requestTransformer) {
    try {
      IndexRequest request = new IndexRequest(index);
  ...
  default SearchResponse search(
      Function<SearchRequest, SearchRequest> requestTransformer, String... indexes) {
  ...
  @SneakyThrows
  default SearchResponse fuzzyQuery(
      String key, Object value, Fuzziness fuzziness, float boost, String... indexes) {
    return transformAndSearch(
        searchSourceBuilder ->
            searchSourceBuilder
                .query(QueryBuilders.fuzzyQuery(key, value).fuzziness(fuzziness).boost(boost))
  ...
  default SearchResponse termQuery(String key, Object term, String... indexes) {
    return transformAndSearch(
        searchSourceBuilder ->
            searchSourceBuilder.query(QueryBuilders.termQuery(key, term)).size(DEFAULT_SEARCH_SIZE),
        indexes);
  }

  @SneakyThrows
  default SearchResponse scriptQuery(
      String script, Map<String, Object> scriptParams, String... indexes) {
  ...
  default SearchResponse matchQuery(String key, Object text, String... indexes) {
    return transformAndSearch(
        searchSourceBuilder ->
            searchSourceBuilder
                .query(QueryBuilders.matchQuery(key, text))
                .size(DEFAULT_SEARCH_SIZE),
        indexes);
  }

  @SneakyThrows
  default SearchResponse transformAndSearch(
      Function<SearchSourceBuilder, SearchSourceBuilder> transformer, String... indices) {
    return search(
        request ->
            request.indices(indices).source(transformer.apply(SearchSourceBuilder.searchSource())),
        indices);
  }
  ...
  default ActionListener<AcknowledgedResponse> getDefaultAcknowledgeCallback() {
    return ActionListener.wrap(
        response -> LOGGER.info("acknowledge: " + response.isAcknowledged()),
        e -> LOGGER.info("error during deletion", e));
  }

  default Function<IndexRequest, IndexRequest> defaultIndexRequest(String id, String json) {
    if (Stream.of(id, json).anyMatch(String::isEmpty))
      throw new RuntimeException("id and json cannot be empty");
    return request -> request.id(id).source(json, XContentType.JSON);
  }
}
