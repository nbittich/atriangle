...
public class ProjectRestController
    implements PingControllerTrait, BuildInfoControllerTrait, ProjectRestFeignClient {
  ...
  public ProjectRestController(
      ProjectRdfService projectRdfService,
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<ProjectEvent> addRawFile(MultipartFile multipartFile, String projectId) {
    return projectFileService
        .addFile(projectId, multipartFile, FileEventType.PROJECT_FILE)
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<ProjectEvent> addRdfFile(MultipartFile multipartFile, String projectId) {
    return projectFileService
        .addFile(projectId, multipartFile, FileEventType.RDF_FILE)
        .map(Map.Entry::getValue)
        .map(ResponseEntity::ok)
        .orElseGet(ResponseEntity.badRequest()::build);
  }
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<ProjectEvent> addShaclFile(MultipartFile multipartFile, String projectId) {
    return projectFileService
        .addFile(projectId, multipartFile, FileEventType.SHACL_FILE)
        .map(Map.Entry::getValue)
        .map(ResponseEntity::ok)
        .orElseGet(ResponseEntity.badRequest()::build);
  }
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<ProjectEvent> addFreemarkerSparqlTemplate(
      MultipartFile multipartFile, String projectId, SparqlQueryRequestType requestType) {

    String extension = FileHelper.getExtension(multipartFile.getOriginalFilename()).orElse("N/A");
    if (!extension.equals("ftl")) {
      log.info(
          "file extension not valid: {}, file name {}, original file name {}",
          extension,
    ...
    return projectFileService
        .addFile(projectId, multipartFile, FileEventType.FREEMARKER_TEMPLATE_FILE)
        .map(
            entry -> {
              ...
              return project
                  ...
                  .sparqlQueries(
                      Stream.concat(
                              project.getSparqlQueries().stream(),
                              Stream.of(
  ...
  public ResponseEntity<ProjectEvent> addSkosFile(MultipartFile file, String projectId) {
    ...
    return projectFileService
        .addFile(projectId, file, FileEventType.SKOS_FILE)
        .map(Map.Entry::getValue)
        .map(ResponseEntity::ok)
        .orElseGet(ResponseEntity.badRequest()::build);
  }
  ...
  public ResponseEntity<SparqlQueryResponse> executeSparqlQuery(SparqlQueryRequest queryRequest) {
    ...
    String queryTempl =
        projectRdfService.getCachedQueryTemplate(
            projectEvent, queryRequest.getFreemarkerTemplateFileId());
    ...
    switch (queryRequest.getType()) {
      ...
      case CONSTRUCT_QUERY:
        return ResponseEntity.ok(
            SparqlQueryResponse.builder()
                .response(
                    projectRdfService.executeConstructSparqlQuery(projectEvent, query, cacheKey))
                .build());
      default:
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<ByteArrayResource> downloadFile(String projectId, String fileId) {
  ...
  @Override
  public ResponseEntity<ProjectEvent> updateProjectDescription(
  ...
  public ResponseEntity<ProjectEvent> skosConversion(
      ...
      String xlsFileEventId) {
    FileEvent xlsFileEvent =
        projectFileService
            .getFileMetadata(projectId, xlsFileEventId)
            .orElseThrow(() -> new RuntimeException("file  not found"));
    if (!FileEventType.SKOS_FILE.equals(xlsFileEvent.getEventType())
        || !CommonConstants.XLSX_MEDIA_TYPE.equals(xlsFileEvent.getContentType())) {
      log.error("only xlsx type supported, provided {}", xlsFileEvent.getContentType());

      throw new RuntimeException("only xlsx type supported");
    }
    return projectRdfService
        .skosConversion(projectId, labelSkosXl, ignorePostTreatmentsSkos, xlsFileEvent)
        .map(ResponseEntity::ok)
        .orElseGet(ResponseEntity.badRequest()::build);
  }
