...
public class FileUploadController
    implements BuildInfoControllerTrait, PingControllerTrait, FileRestFeignClient {
  ...
  public FileUploadController(
      FileUploadService uploadService, LoggerAction loggerAction, BuildProperties buildProperties) {
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<ByteArrayResource> download(String id, String correlationId)
      throws Exception {
    Optional<FileUpload> upload = uploadService.findOneById(id);
    return upload.map(FileUpload::transform).stream()
        .peek(
            event ->
                loggerAction.info(
                    () -> correlationId,
                    "Download request: %s, name: %s, content-type: %s, event type: %s ",
                    event.getId(),
                    event.getName(),
        ...
        .map(
            event ->
                RestUtil.transformToByteArrayResource(
                    event, uploadService.uploadToByteArray(event)))
        .findFirst()
        .orElseGet(ResponseEntity.notFound()::build);
  }
  ...
  @Override
  @SwaggerHeaderAuthentication
  public ResponseEntity<FileEvent> upload(
      MultipartFile file, FileEventType fileUploadType, String correlationId) throws Exception {
    return Optional.of(uploadService.upload(file, fileUploadType)).stream()
        .peek(
            event ->
                loggerAction.info(
                    () -> correlationId,
                    "Upload request: %s, name: %s, content-type: %s, event type: %s ",
                    event.getId(),
                    event.getName(),
                    event.getContentType(),
                    event.getEventType()))
